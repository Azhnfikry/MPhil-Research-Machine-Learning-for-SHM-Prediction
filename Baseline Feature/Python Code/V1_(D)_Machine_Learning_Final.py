...
from sklearn.model_selection import (
    learning_curve, validation_curve, cross_val_score, train_test_split, KFold,
)
...
try:
    import model_structure_plots
    MODEL_STRUCTURE_PLOTS_OK = True
except Exception:
    MODEL_STRUCTURE_PLOTS_OK = False
...
INPUT_TRAIN_DIR = r"C:\Users\N O\Desktop\MASTER\Methodology & Result\Pre-Final Dataset\Dataset\train_dataset_30minute_80pct_chronological.csv"
INPUT_TEST_DIR  = r"C:\Users\N O\Desktop\MASTER\Methodology & Result\Pre-Final Dataset\Dataset\test_dataset_30minute_20pct_chronological.csv"
BASE_OUTPUT_DIR = r"C:\Users\N O\Desktop\MASTER\Methodology & Result\Pre-Final Dataset"
...
# cross_val_score(cv=<int>) defaults to an UNSHUFFLED KFold, which on this
# already-chronological train fold carves 3 contiguous time blocks. If the
# signal drifts over time, a flexible model that fits one block's local
# trend scores badly on a different-regime block, while a near-constant
# model dodges that penalty everywhere -- so the tuning objective itself
# rewards flat/degenerate hyperparameters (observed: XGBoost's reg_alpha
# kept getting pushed to whatever ceiling was available, however low).
# Shuffling here only reorders which TRAIN rows fall into which internal
# CV fold for hyperparameter search -- the outer train/test split (and its
# chronological ordering) is untouched, so this doesn't leak the test set.
TUNE_CV = KFold(n_splits=TUNE_CV_FOLDS, shuffle=True, random_state=OPTUNA_SEED)
...
def tune_random_forest(X_train, y_train, n_trials):
    def objective(trial):
        ...
        return cross_val_score(pipe, X_train, y_train,
                               cv=TUNE_CV, scoring=TUNE_METRIC, n_jobs=1).mean()
...
def tune_xgboost(X_train, y_train, n_trials):
    def objective(trial):
        params = dict(
            ...
            # reg_alpha (L1) zeroes a leaf outright whenever the leaf's
            # summed gradient falls below it; reg_lambda (L2) shrinks the
            # leaf weight toward 0 as it grows relative to summed hessian.
            # Both are absolute thresholds in gradient/hessian units, and
            # this target's tiny, quantized scale (~0.0125 Hz steps) keeps
            # per-leaf gradient sums well under 1 -- a ceiling of 10.0 let
            # trials zero every leaf in every tree (reg_alpha~=5.5 observed
            # in practice), collapsing the whole boosted ensemble to the
            # target's mean regardless of learning_rate/n_estimators.
            # Capped an order of magnitude lower, same fix as gamma above.
            reg_alpha        = trial.suggest_float("reg_alpha", 1e-4, 1.0, log=True),
            reg_lambda       = trial.suggest_float("reg_lambda", 1e-4, 1.0, log=True),
            # gamma is a minimum-loss-reduction threshold in absolute
            # squared-error units. This target's values are quantized
            # (~0.0125 Hz steps), so split gains sit around 1e-4 -- even
            # the top of a [1e-6, 0.05] range was still large enough to
            # starve every split and collapse the tuned model to a near-
            # constant predictor (flat prediction line, R2~=0 observed in
            # practice). Capped an order of magnitude lower so the ceiling
            # stays below the smallest gains this target actually produces.
            gamma            = trial.suggest_float("gamma", 1e-6, 0.01, log=True),
        )
        pipe = Pipeline([
            ("imp", SimpleImputer(strategy="median")),
...
def tune_svr(X_train, y_train, n_trials):
    ...
    # normalised fraction -- a flat [1e-5, 1.0] range let trials pick an
    # epsilon-insensitive tube (e.g. 0.145) several times wider than this
    # target's entire value range (~0.0375 Hz for X/Y Frequency), so the
    # tube swallowed all real variation and the "tuned" model degenerated
    # into predicting a near-constant line. Scale the ceiling to this
    # target's own std so the tube can never exceed the signal it's fit to.
    ...
    def objective(trial):
        ...
        epsilon = trial.suggest_float("epsilon", 1e-5, eps_hi, log=True)
...
for train_file, test_file, resample_tag in DATASETS:
    ...
    try:
        ...
        for target_label, y_train, y_test in [
            ...
            if MODEL_STRUCTURE_PLOTS_OK:
                try:
                    model_structure_plots.plot_all_model_structures(
                        fitted_models, family_X_train, family_X_test, y_train, y_test,
                        target_label, pfx, FEATURE_LABELS_BY_FAMILY,
                    )
                except Exception as e:
                    print(f"    Warning: model structure plots failed: {e}")
            else:
                print("    Skipping model structure plots: model_structure_plots import failed.")
